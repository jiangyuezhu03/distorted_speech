...
def generate_mask(speech, masker, sr, filters,threshold):# change noise/music to masker

    speech_bands = erb_filterbank(speech, filters)
    noise_bands = erb_filterbank(masker, filters)
...
def apply_mask_and_resynthesize(mixture, mask, sr, filters, tri_win): # function name change only
    channels, time_frames = mask.shape
    samples_per_frame = int(sr / 100)  # 10 ms frame step
    win_len = len(tri_win)
    half_win = win_len // 2
...
def glimpsed_speech(speech, sr, filters, tri_win):
    ...
    mask = generate_mask(speech, shaped_noise, sr, filters,3)
    glimpsed = apply_mask_and_resynthesize(mixture, mask, sr, filters, tri_win)
    return glimpsed
# added this function
...
def sculpted_speech(speech, sr, music_wave, filters,tri_win):
    music_fragment = segment_music(music_wave, speech, sr)
    music_fragment = match_snr(speech,music_fragment,target_snr_db=0)

    ssn = generate_speech_shaped_noise(speech, sr)
    ssn = match_snr(speech,ssn,target_snr_db=0)
    mask = generate_mask(speech, ssn, sr, filters,3) # threshold =-6 in original paper, change to +3

    sculpted = apply_mask_and_resynthesize(music_fragment, mask, sr, filters, tri_win)
    return sculpted
# added this configurer
def prepare_sculpted_config(sr=16000, win_len_ms=20, channels=55, f_min=50, f_max=7500):
    music_wave, _ = librosa.load("music_bach.mp3", sr=sr)
    centre_freqs = genenerate_centre_freqs(sr, channels, f_min, f_max)
    filters = make_erb_filters(sr, centre_freqs)
    win_len = int((win_len_ms / 1000) * sr)
    tri_win = triang(win_len)
    return filters, tri_win, music_wave
...
def apply_distortion_old(ds, distortion_type):
    # sr must be 16000
    distortion_func = None
    distortion = distortion_type

    # Define default apply_function (2 args: waveform, sr)
    def apply_function(waveform, sr):
        return waveform  # fallback identity
    ...
    if distortion_type == "fast":
        ...
        filters, tri_win, music_wave = prepare_sculpted_config(sr=16000)
        distortion_func = lambda waveform, sr: sculpted_speech(np.array(waveform), sr, music_wave, filters,tri_win)
    ...
    gen_new_dataset(ds, distortion, distortion_func)

# does not work for train_5000
def gen_new_dataset(original_ds, distortion_type,apply_function):
    outpath=f"ted3train_distorted/{distortion_type}"
...
def apply_distortion(ds, distortion_type):
    # sr must be 16000
    distortion_func = None
    ...
    if distortion_type == "fast":
        ...
        filters,tri_win,music_wave = prepare_sculpted_config(sr=16000)
        distortion_func = lambda waveform, sr: sculpted_speech(waveform, sr, music_wave, filters, tri_win)
    elif distortion_type == "glimpsed":
        filters, tri_win = prepare_glimpsed_config(sr=16000)
        distortion_func = lambda waveform, sr: glimpsed_speech(waveform, sr, filters, tri_win)
    elif distortion_type == "clean":
    ...
    # Define a function compatible with map()
    def process_sample(sample):
        transcript = sample["text"].strip().lower()

        if "ignore_time_segment_in_scoring" in transcript or "inter_segment_gap" in transcript:
            return None  # will be removed if remove_columns is False

        waveform = sample["audio"]["array"]
        sr = sample["audio"]["sampling_rate"]

        new_waveform = distortion_func(waveform, sr)
        scaled_new_waveform = scale_tofactor(distortion_type, new_waveform)

        # Return modified audio array inside the audio dict
        sample["audio"]["array"] = scaled_new_waveform
        return sample

    # Apply with map
    distorted_ds = ds.map(process_sample, remove_columns=[], desc=f"Applying {distortion_type}", num_proc=4)

    # Filter out None entries (skipped ones)
    distorted_ds = distorted_ds.filter(lambda x: x is not None)

    # outpath = f"ted3train_5000_distorted/{distortion_type}"
    outpath=f"ted3test_distorted/{distortion_type}"
    os.makedirs(outpath, exist_ok=True)
    distorted_ds.save_to_disk(outpath)
    print(f"New dataset saved to {outpath}")


if __name__ == "__main__":
    # my_ds = load_from_disk("ted3train_5000_distorted/cleaned")
    my_ds = load_from_disk("ted3test_distorted/clean")
    print(len(my_ds))
    distortion_type = sys.argv[1].lower()
    apply_distortion_old(my_ds,distortion_type)